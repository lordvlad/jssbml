...
module.exports = class Writer extends Transform {
  ...
  _writeModel (m) {
    ...
    this._writeListOf('Compartments', values(m.compartments), this._writeCompartment)
    this._writeListOf('Species', values(m.species), this._writeSpecies)
    this._writeListOf('Reactions', values(m.reactions), this._writeReaction)
    this._in(1)
    this._close('model')
  }
  ...
  _writeSbml (document) {
    this._open('sbml', pick(document, 'xmlns', 'level', 'version'))
    this._nl()
