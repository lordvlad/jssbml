...
module.exports = class Writer extends Transform {
  ...
  _writeAnnotation (annotation, depth = 1) {
    ...
    this._nl()
    if (annotation.listOfLayouts) {
      this._writeLayouts(annotation.listOfLayouts)
      this._nl()
    }
  ...
  _writeReactionGlyph (c) {
    ...
    this._close('curve')
    this._nl()
    this._writeListOf('SpeciesReferenceGlyphs', c.speciesReferenceGlyphs, this._writeSpeciesReferenceGlyph, 6)
    this._in(5)
  ...
  _writeReaction (r) {
    ...
    this._nl()
    this._writeListOf('Reactants', r.listOfReactants, this._writeSpeciesReference, 4)
    this._writeListOf('Products', r.listOfProducts, this._writeSpeciesReference, 4)
    this._writeListOf('Modifiers', r.listOfModifiers, this._writeSpeciesReference, 4)
    this._in(3)
  ...
  _isEmpty (x) {
    return (('size' in x && x.size === 0) || ('length' in x && x.length === 0))
  }

  _writeListOf (what, it, single, depth = 2) {
    if (this._isEmpty(it)) return
    this._in(depth)
