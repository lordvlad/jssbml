...
const {CompartmentGlyph, Layout, BBox,
  ...
  ReactionGlyph, Model, ModifierSpeciesReference,
  Annotation, ListOfLayouts, ListOfCompartmentGlyphs,
  ListOfReactionGlyphs, ListOfSpeciesGlyphs,
  ListOfTextGlyphs
} = Document
...
function makeAnnotation (node, parent) {
  ...
  if (layoutNodes && layoutNodes.length) {
    annotation.listOfLayouts = new ListOfLayouts()
    for (let s of layoutNodes) annotation.listOfLayouts.push(makeLayout(s, parent))
  }
  ...
  return isEmptyObject(annotation) ? undefined : assign(new Annotation(), annotation)
}
...
function makeLayout (node, { reactions, compartments, species, speciesReferences }) {
  ...
  for (let s of listOf(node, 'ReactionGlyphs')) {
    const c = makeReactionGlyph(s, { glyphs, reactions, speciesReferences })
  ...
  // layout.listOfTextGlyphs.sort()
  // layout.listOfReactionGlyphs.sort()
  // layout.listOfSpeciesGlyphs.sort()
  // layout.listOfCompartmentGlyphs.sort()
...
function makeSpeciesReferenceGlyph (node, {speciesReferences, glyphs}) {
...
function makeReactionGlyph (node, {reactions, speciesReferences, glyphs}) {
  ...
  for (let s of listOf(node, 'SpeciesReferenceGlyphs')) {
    const spec = makeSpeciesReferenceGlyph(s, {glyphs, speciesReferences})
    reaction.listOfSpeciesReferenceGlyphs.push(spec)
  }
  // reaction.speciesReferenceGlyphs.sort()
  return reaction
}
...
function makeReaction (node, ctx) {
  ...
  function mklist (a) {
    const n = `listOf${a}`
    const m = `ListOf${a}`
    const Ctor = eval(`Document.${m}`) // eslint-disable-line no-eval
    return makeListOf2(getChild(node, n), ctx, 'speciesReferences', Ctor, mkref)
  }

  const listOfReactants = mklist('Reactants')
  const listOfProducts = mklist('Products')
  const listOfModifiers = mklist('Modifiers')
...
function makeSpeciesReference (node, {species}) {
  const x = node.$name === 'speciesReference' ? new SpeciesReference() : new ModifierSpeciesReference()
  return assign(x, {
    species: species[node.species],
